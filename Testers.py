...
translated_text = arabicTransformers.translation(model, "أهلا، كيف حالك؟") #2- You still need to call the model (the Class Constructor) here too.
print(translated_text, '\n\n')


print("# English Masked Language Modeling:")
model = arabicTransformers("xlm-roberta-large")
masked_text = arabicTransformers.fill_mask(model, "Hello, I'm a <mask> model.") #2- You still need to call the model (the Class Constructor) here too.
print(masked_text, '\n')

print("# Arabic Masked Language Modeling:")
model = arabicTransformers("CAMeL-Lab/bert-base-arabic-camelbert-mix")
masked_text = arabicTransformers.fill_mask(model, "اللغة العربية هي لغة [MASK].") #2- You still need to call the model (the Class Constructor) here too.
print(masked_text, '\n\n')
...
ner_text = arabicTransformers.token_classification(model, "My name is Ali and I live in Dubai") #2- You still need to call the model (the Class Constructor) here too.
print(ner_text, '\n')
...
pos_text = arabicTransformers.token_classification(model, "إمارة أبوظبي هي إحدى إمارات دولة الإمارات العربية المتحدة السبع") #2- You still need to call the model (the Class Constructor) here too.
print(pos_text, '\n\n')
...
result = arabicTransformers.question_answering(model, question=question, context=context) #2- You still need to call the model (the Class Constructor) here too.
print(result, '\n')
...
'''
summarized_text = arabicTransformers.summarization(model, text=text) #2- You still need to call the model (the Class Constructor) here too.
...
text_classified = Sarsor.text_classification(model, "I like you. I love you.") #2- You still need to call the model (the Class Constructor) here too.
...
text_classified = Sarsor.text_classification(model,"كل عام وانت طيب يا باشا") #2- You still need to call the model (the Class Constructor) here too.
print(text_classified, '\n\n')


print("# English Text Generation:")
model = arabicTransformers('gpt2') 
text_generated = arabicTransformers.text_generation(model, "My name is Julien and I like to") #2- You still need to call the model (the Class Constructor) here too.
print(text_generated, '\n')

print("# Arabic Text Generation:")
model = arabicTransformers('aubmindlab/aragpt2-base') 
text_generated = arabicTransformers.text_generation(model, "القدس مدينة تاريخية، بناها الكنعانيون في") #2- You still need to call the model (the Class Constructor) here too.
print(text_generated, '\n\n')
